...
int main(int argc, char* argv[]) {
  if (argc != 4) {
    ...
    int cnt=0;
    for(int i=0;i<42;i++){
        printf("%02x ",my_arp_request_packet[i]);
        cnt++;
        if(cnt==16) {
            cnt=0;
            printf("/n");
    ...
    while (1) {
        ...
        int cnt1=0;
        for(int i = 0; i < 42; i++){
            printf("%02x ", my_arp_reply_packet[i]);
            cnt1++;
            if(cnt1==16) {
                cnt1=0;
                printf("\n");
